...
class SignUpCubit extends Cubit<SignUpState> {
  ...
  SignUpCubit(this.repository,this._authentication,this.snackBarCubit,this.loadingCubit) : super(SignUpState.initial());

  Future<void> registerUser(String email,String password,String confirmPassword,String displayName)async{
    if( password==confirmPassword){
      loadingCubit.showLoading();
      try{
        UserCredential userCredential = await _authentication.signUpWithEmailAndPassword(email, password,displayName);
        User? userDetails = userCredential.user;
        UserModel newUser = UserModel(
            id: userDetails!.uid,
            email: userDetails.email,
            displayName: displayName,
            imgUrl:  'https://9mobi.vn/cf/images/ba/2018/4/16/anh-avatar-dep-1.jpg');
        ...
        if (e.code == 'weak-password') {
          ...
          loadingCubit.hideLoading();
          snackBarCubit.showSnackBar(SnackBarType.error, " user name already in use");
          emit(SignUpState.initial());
        }else{
          loadingCubit.hideLoading();
      ...
      snackBarCubit.showSnackBar(SnackBarType.error, "Password and Confirm Password are different");
      return;
    }
  }
}
